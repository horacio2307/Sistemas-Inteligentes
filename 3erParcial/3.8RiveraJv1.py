...
def Valor_Error(Vector_F):

    Vector=[0,0,0,0,0]
    Vector=sorted(Vector_F,reverse=True)
    print("\nEl error es: {}".format(abs(Vector[4]-Vector[0])))
    return abs(Vector[4]-Vector[0])
...
while (error >=error_referencia and h<limite):
    clear()
    NuDecimal=Convertir(numeros) #5 numeros en decimal 
    if(h==0):
        for i in range(0,5): #Imprimir los numeros en formato tabla
            FuncionF[i]=f(NuDecimal[i])                        
            print("Cromosomas {}: {}\t\tNumero x: {}\t\t Valor de f(x): {}".format(i+1,numeros[i],NuDecimal[i],FuncionF[i]))
    ...
    print("\nLos individuos menos aptos son los numeros {} y {}".format(Vector_Menos_Aptos[0]+1,Vector_Menos_Aptos[1]+1))
    ...
    #Elitismo
    for x in range(0,12):
        numeros[Vector_Menos_Aptos[0]][x]=bebe1[x]
        numeros[Vector_Menos_Aptos[1]][x]=bebe2[x]
    
    print("\nGeneracion ",h+2)
    print("La nueva generacion es la siguiente")
