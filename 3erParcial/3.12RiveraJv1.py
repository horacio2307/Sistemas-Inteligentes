...
def MenosAptos():
    ...
    state = False
    #print(Aciertos)
    while(i<4 and state==False):

        for j in range(0,NuIndividuos):
            if(Aciertos[j]==i and x1==-1):
                aux1=j
                x1=0

        for j in range(0,NuIndividuos):
            if(Aciertos[j]==i and x2==-1 and j!=aux1):
                aux2=j
                x2=0
...
pm=0.3
m=20
#m=2*random.uniform(-1,1)

iteracion = 1 #Valor del ciclo
Limite = 1000 #Limite de paro
...
while iteracion < Limite :
    ...
    #Sino el programa sigue
        ...
        print("Los padres son los individuos {} y {} \n".format(Padres[0]+1,Padres[1]+1))

        #Cruce-------------------------------------
        Gen_Cruza = random.randint(1,11)
        print("Se cruzara en el gen ",Gen_Cruza)
        for i in range(0,13):
            if(i<Gen_Cruza):
        ...
        if(a1<pm):
            Gen_Muta1 = random.randint(0,12)
            print("\nEl bebe 1 mutara en el gen ",Gen_Muta1+1)
            Bebes[0][Gen_Muta1]+=m

        a2=random.uniform(0,1)
        if(a2<pm):
            Gen_Muta2 = random.randint(0,12)
            print("\nEl bebe 2 mutara en el gen ",Gen_Muta2+1)
            Bebes[0][Gen_Muta2]+=m
        elif(a1>pm):
            print("\nNo hay mutacion")
        #------------------------------------------
        ...
        MeApto1,MeApto2=MenosAptos()
        print("\nLos individuos menos aptos son {} y {}".format(MeApto1+1,MeApto2+1))
        #Elitismo----------------------------------
        for x in range (0,13):
            poblacion[MeApto1][x]=Bebes[0][x]
            poblacion[MeApto2][x]=Bebes[1][x]            
        #------------------------------------------
        #Aumenta la iteracion
        iteracion+=1

print("La poblacion tiene los siguientes aciertos")
print(Aciertos)
print("Generaciones totales ",iteracion)
